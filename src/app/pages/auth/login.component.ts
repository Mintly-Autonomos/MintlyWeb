      ...
      <form (ngSubmit)="onSubmit()" class="space-y-4">
        ...
        <div class="flex items-center justify-between pt-1">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" [(ngModel)]="remember" name="remember" class="h-4 w-4 rounded border-border accent-mint" />
            <span class="text-[13px] text-foreground/80">Manter conectado</span>
          </label>
          <a routerLink="/auth/esqueci-senha" class="text-[13px] text-ocean font-medium hover:underline">Esqueci minha senha</a>
        </div>

        <button type="submit" [disabled]="loading()" class="w-full h-12 rounded-xl bg-mint text-primary-foreground font-semibold text-sm hover:brightness-95 disabled:opacity-50 transition flex items-center justify-center gap-2 cursor-pointer">
          @if (loading()) {
        ...
        <button type="button" (click)="router.navigate(['/auth/cadastro'])" class="w-full h-12 rounded-xl border border-border bg-card text-foreground font-medium text-sm hover:bg-muted transition flex items-center justify-center gap-2 cursor-pointer">
        ...
        <p class="flex items-start gap-2 text-[12px] text-muted-foreground pt-1">
          <app-icon name="shield" [style]="{fontSize:'16px'}" className="text-mint mt-0.5 shrink-0" />
          <span>Para sua segurança, nunca compartilhamos seus dados de acesso.</span>
        </p>
      </form>

      <div slot="footer">
        Não tem conta? <a routerLink="/auth/cadastro" class="text-ocean font-semibold hover:underline">Criar gratuitamente</a>
      </div>
    </app-auth-card>
  `,
})
