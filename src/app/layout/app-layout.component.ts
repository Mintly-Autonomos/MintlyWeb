...
@Component({
  ...
  template: `
    <div class="min-h-screen w-full flex bg-background text-foreground">
      <!-- Sidebar -->
      <aside class="w-[260px] shrink-0 hidden md:flex flex-col border-r border-border bg-sidebar sticky top-0 h-screen">
        <div class="px-6 py-5">
          <app-logo [size]="30" tagline="Gestão para o seu negócio" to="/" />
        </div>

        <div class="px-3 mt-2 mb-1 text-[11px] uppercase tracking-wider text-muted-foreground/80 font-semibold">Menu</div>
        ...
        <nav class="px-2 flex-1 space-y-1">
          @for (item of navItems; track item.to) {
            ...
            >
              <span
                [class]="'material-symbols-outlined ' + (rla.isActive ? 'text-mint' : 'text-muted-foreground')"
                [style.fontSize]="'20px'"
        ...
        <div class="m-3 p-4 rounded-2xl bg-mint-soft border border-border">
          ...
          <p class="text-xs text-foreground/70 mt-1.5 leading-relaxed">
            Centralize bancos, caixas e plataformas em um só lugar para ter visão real do seu dinheiro.
          </p>
        </div>
      </aside>
      ...
      <div class="flex-1 min-w-0 flex flex-col">
        <header class="sticky top-0 z-20 h-[68px] border-b border-border bg-background/85 backdrop-blur flex items-center px-6 md:px-10 gap-4">
          <div class="min-w-0">
            <h1 class="text-[20px] md:text-[22px] font-bold tracking-tight truncate">{{ pageTitle }}</h1>
            <p class="text-[13px] text-muted-foreground truncate">{{ pageSub }}</p>
          </div>
          <div class="ml-auto flex items-center gap-2">
            <div class="hidden lg:flex items-center gap-2 h-10 px-3 rounded-full border border-border bg-card w-[280px]">
              <app-icon name="search" className="text-muted-foreground" [style]="{fontSize:'18px'}" />
              <input placeholder="Buscar movimentação, conta…" class="bg-transparent outline-none text-sm flex-1 placeholder:text-muted-foreground" />
              <kbd class="text-[10px] text-muted-foreground border border-border rounded px-1.5 py-0.5">⌘K</kbd>
            </div>
            <button
            ...
            <button class="h-10 w-10 rounded-full border border-border grid place-items-center hover:bg-muted transition relative cursor-pointer">
              <app-icon name="notifications" [style]="{fontSize:'20px'}" />
              <span class="absolute top-2 right-2.5 h-2 w-2 rounded-full bg-mint ring-2 ring-background"></span>
            </button>
            <a
...
export class AppLayoutComponent {
  ...
  private readonly titles: Record<string, { title: string; sub: string }> = {
    ...
    '/movimentacoes': { title: 'Movimentações', sub: 'Veja, registre e organize entradas e saídas.' },
    '/categorias': { title: 'Categorias', sub: 'Organize suas receitas e despesas por categoria.' },
  };

  private router = inject(Router);

  get pageTitle(): string { return this.titles[this.router.url]?.title ?? 'Mintly'; }
  get pageSub(): string { return this.titles[this.router.url]?.sub ?? ''; }
  fillVar(active: boolean): string { return active ? "'FILL' 1" : ''; }
}
